...
function policyInit() {
	...
	// adding JSON parser when browser is too old to have a build-in one
	// (pre-IE8, pre-FF3.5, ...)
	if (typeof (window['JSON']) == "undefined")
		document
				.write('<script type="text/javascript" src="https://github.com/douglascrockford/JSON-js/raw/master/json2.js"/>');
	...
	$(function() {
		...
		$.datepicker
				.setDefaults({
					...
					changeYear : true,
					yearRange : (''
							+ (IMPACT.jsloadstarted.getFullYear() - 100) + ':' + (IMPACT.jsloadstarted
							.getFullYear() + 10)),
					showAnim : 'slideDown'
				});
		...
		$("#locate").change(
				function() {
					...
					// translates labels
					translate([ "choose_topic", "topics", "hints", "questions",
							"solution", "next", "orphan_works" ], lang,
					...
					if (IMPACT.current_questions.length > 0) {
						...
						send_data(simulation_url(), {
							...
							show_questions(data, false, true);
						});
					}
							...
							$(this)
									.click(
											function() {
												$(
														document
																.getElementsByTagName("h3")[li_i].firstChild)
														.click();
											});
						});
		...
		$.ajaxSetup({
			...
			complete : function(XMLHttpRequest, textStatus) {
				if (textStatus == "success")
					...
					// "notmodified", "timeout", or "parsererror"
...
function simulation_url() {
	return IMPACT.debug ? "/PolicySimulation"
			: "/PolicyModellingTool2/PolicySimulation";
}

function evaluation_url() {
	return IMPACT.debug ? "/PolicyEvaluation"
			: "/PolicyModellingTool2/PolicyEvaluation";
}
...
function load_topic(t) {
	send_data(simulation_url(), {
		...
		show_questions(data, true, false);
	});
}
...
function show_policy(policy, stmts_ids) {
	...
	var svg = $('svg').svg('get');
	var corner = 5;
	svg.rect(g, x, y, w, h, corner, corner, {
		"stroke-width" : 2,
		stroke : "purple",
		fill : "transparent"
	});
...
function show_questions(data, is_first_display, lang_changed) {
	// {"questions":[{"answers":[],"formalanswers":null,"id":1,"category":"Name","optional":false,"hint":"Please
	// enter the name of the rights owner.","type":"text","question":"What is
	// the name of the right owner?","statement":["hatName","?Person"]},
	// {"answers":[],"formalanswers":null,"id":2,"category":"Work","optional":false,"hint":null,"type":"text","question":"What
	// is the name of the concerned orphaned
	// work?","statement":["betrifftWerk","?x"]} ]}
	...
	if (is_first_display) {
		IMPACT.displayed_categories.push({
			category : category,
	...
	// remove previous title if it exits (for instance after asking a change in
	// the language)
	$('#questionlist').empty();
	// add new div to questionlist with id = topic name
	// TODO: what is this qcontent id use for?
	qlist.append('<div id="{0}"><h3>{1}</h3><div id="qcontent"></div></div>'
	...
	// qbox.append('<input type="button" class="ui-button next" value="next"
	// onclick="send_answers(\''+category+'\')"/>');
	var buttonId = gen_id();
	qbox
			.append('<input type="button" id="'
					+ buttonId
					+ '" class="ui-button ui-widget ui-state-default ui-corner-all" value="next" />');
	translate([ "next" ], $('#locate').val(), function(translations) {
 ...
 * 
 * @param {object}
 *            question json object representing the question
 * @param {object}
 *            qcontent div inside topic div
 */
function show_question(item, qbox) {
	var newline = false;
	// pre append formatting
	var output = "<p><label for=\"qID" + item.id + "\">" + item.question
			+ "</label>";
	// select
	if (item.type == "select") {
		output += "<select class=\"required\" id=\"qID" + item.id
				+ "\" name=\"qID" + item.id + "\">";
		output += "<option value=\"\" selected=\"selected\">-- please choose --</option>";

		$.each(item.answers, function(answindex, answer) {
			output += "<option value=\"" + answer + "\">" + answer
					+ "</option>";
		});
		output += "</select>";
		...
		for ( var i = 0; i < item.answers.length; i++) {
			...
			output += "<input class=\"required\" name=\"qID" + item.id
					+ "\" type=\"" + item.type + "\" value=\"" + formalanswer
					+ "\">";
			output += "<span onclick=\"$(this).prev().click()\">" + answer
					+ "</span>";
		}
	} else if (item.type == "date") {
		output += "<input type=\"text\" class=\"datefield required\" id=\"qID"
				+ item.id
				+ "\" name=\"qID"
				+ item.id
				+ "\""
		...
		output += "<input type=\"text\" class=\"integer required\" id=\"qID"
				+ item.id
				+ "\" name=\"qID"
				+ item.id
				+ "\""
		...
		output += "<input type=\"text\" class=\"float required\" id=\"qID"
				+ item.id
				+ "\" name=\"qID"
				+ item.id
				+ "\""
		...
		output += "<input class=\"required\" type=\""
				+ item.type
				+ "\" id=\"qID"
				+ item.id
				+ "\" name=\"qID"
				+ item.id
				+ "\""
				+ ((item.answers && item.answers.length != 0) ? " value=\""
						+ item.answers[0] + "\"" : "") + "/>";
	}
	...
	if (item.hint)
		$("#hints").append(
				"<p id=\"qHINT" + item.id + "\" class=\"hint\">" + item.hint
						+ "</p>");
	// post append formatting
	if (item.optional)
		$("p:last :input:first", qbox).addClass("optional");
	...
	if (item.type != "radio" && item.type != "checkbox") {
		...
		$("input:last", qbox).parent().mouseover(
				function() {
					if (IMPACT.showhints) {
						var hinton = $("#hints > p:not(:hidden)");
						// update_status(1,"Verstecke: "+((hinton.length >
						// 0)?"#qID"+hinton.attr("id").substring(5):"-")+" |
						// Zeige:
						// "+"#qHINT"+$(this).children("input:first").attr("name").substring(3));
						if (hinton.length > 0)
							$("#qID" + hinton.attr("id").substring(5)).blur();
						$(
								"#qHINT"
										+ $(this).children("input:first").attr(
												"name").substring(3)).show();
					}
				});
		$("input:last", qbox).parent().mouseout(
				function() {
					$(
							"#qHINT"
									+ $(this).children("input:first").attr(
											"name").substring(3)).css(
							'display', 'none');
				});
		// validation
 ...
 * 
 * @param {string}
 *            solution string representing the main issue
 * @param {string}
 *            path url pointing to solution lkif
 */
 ...
 * 
 * @param {object}
 *            obj a question formfield or a array or DIV-element that contains
 *            this HTML nodes
...
function validate_field(obj) {
	...
	// validation:
			...
			&& (obj.nodeName == "INPUT" || obj.nodeName == "SELECT" || obj.nodeName == "TEXTFIELD")) {
		var o = $(obj);
		// ignore buttons:
		if (obj.type
		...
		if (o.hasClass("integer")) {
			if (o.val().search(/\D/) != -1) {
				qwarn(obj,
						"Please insert a integer. No characters or whitespaces allowed.");
				return false;
			}
		}
		if (o.hasClass("float")) {
			if (o.val() != ""
					&& o.val().search(/[\sa-zA-Z]/i) != -1
					|| (null == /(^-?\d*\.?\d*$)/.exec(o.val()) ? true
							: /(^-?\d*\.?\d*$)/.exec(o.val())[0] !== o.val())) {
		...
		if (o.hasClass("datefield")) {
			if (o.val() != ""
					&& o.val().search(/[\sa-zA-Z]/i) != -1
					|| o.val().search(/\d\d[\.\/]\d\d[\.\/]\d\d\d\d/i) != -1
					&& (null == /(\d\d[\.\/]\d\d[\.\/]\d\d\d\d)/i.exec(o.val()) ? true
							: /(\d\d[\.\/]\d\d[\.\/]\d\d\d\d)/i.exec(o.val())[0] !== o
									.val())) {
		...
		if (obj.type == "radio" || obj.type == "checkbox") {
			if (obj.parentNode.getElementsByTagName("input")[0] == obj
					&& !o.hasClass("optional")) {
			...
			} else { // abort validation because it already has been
						// validated or is optional
				return true;
			}
		}
		// empty field
		...
		alert("nothing to validate on field: " + obj.nodeName + " of type "
				+ $.type(obj));
		return false;
	}
}
 ...
 * 
 * @param {Array}
 *            answers Array that contains all possible answers
 ...
 * 
 * @param {number}
 *            type Expect a integer with the value of the status. 0 means
 *            loading 1 an error and -1 that there is everthing allright so
 *            loaded.
 * @param {string}
 *            text Here goes the Text that will be displayed in the status.
 */
...
/**
 * Collects the given answers and parse them as JSON before sending them to the
 * server.
 * 
 * @param {string}
 *            category the ID of the question div
 * @see validate_field
 */
	...
	$("input", topicDiv)
			.each(
					function(i, itemobj) {
						var item = $(itemobj);
						// skip buttons
						if (item.hasClass("ui-button")
								|| itemobj.type
								&& (itemobj.type == "button"
										|| itemobj.type == "submit" || itemobj.type == "reset"))
							return true;
						...
						if (itemobj.type == "radio"
							 ...
							 * $(":input:checked[name='"+itemobj.name+"']",
							 * itemobj.parentNode).each(function(i, valobj) {
...
function send_abduction_request() {
	send_data(evaluation_url(), {
		abduction : {
			argGraph : IMPACT.arg_graph,
			acceptability : $('input[name=abduction]').val()
		}
	}, show_position);
}

/**
 * Displays a warning besides a form field when invalid data is used. To hide
 * this use {@link qunwarn}
 * 
 * @param {object}
 *            obj triggering form field HTML object
 * @param {string}
 *            warning text that appears right besides the field
 * @see qunwarn
 */
function qwarn(obj, warning) {
	...
	if ($(".qwarn", obj.parentNode).length > 0) {
		...
		if ($("br", obj.parentNode).length > 0) { // newline checkbox/radio
			...
			o.before("<span class=\"qwarn\"><i></i>" + warning
					+ "<b></b></span>");
			o.prev().fadeIn(500);
		} else {
			o = $(":last-child", obj.parentNode);
			o.after("<span class=\"qwarn\"><i></i>" + warning
					+ "<b></b></span>");
			o.next().fadeIn(500);
		}
	} else {
...
function qunwarn(obj) {
	...
	$(".qwarn", obj.parentNode).fadeOut(500, function() {
		$(this).remove();
	});
}
...
function add_category(category_name, category, first_question) {
	...
	$("#categories").append(
			'<li id="' + category_html_id + '">' + category_name + '</li>');

	$('#' + category_html_id).click(function() {
...
function show_policyrules(data) {
	...
	$.each(rules, function(ruleindex, r) {
		policyList.append('<li><input type="checkbox" name="' + r + '" />' + r
				+ '</li>');
		IMPACT.policyrules.push(r);
	});
	$("#policyrules")
			.append(
					'<input type="button" class="ui-state-hover ui-button ui-widget ui-state-default ui-corner-all ui-button ui-widget ui-state-hover evaluate" value="Evaluate" onclick="evaluate_graph()"/>');
 ...
 * 
 * @param {string}
 *            path path to lkif with argument graph
 */
 ...
 * 
 * @param {string}
 *            path path to svg with argument graph
 */
